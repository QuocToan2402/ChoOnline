...
export const signin = (email, password) => async (dispatch) => {
  ...
  try {
    ...
    localStorage.setItem('userInfo', JSON.stringify(data));//set data in localStorage, change array to json.
  } catch (error) {
    //render error or message
    dispatch({
...
export const updateUserProfile = (user) => async (dispatch, getState) => {
  ...
  try {
    ...
    dispatch({ type: USER_SIGNIN_SUCCESS, payload: data });
    localStorage.setItem('userInfo', JSON.stringify(data));//save user change info to local 
  } catch (error) {
    const message =
      error.response && error.response.data.message
        ? error.response.data.message
